...
class AsyncFetcher {
     ...
     */
    func fetchAsync(_ identifier: UUID, imageId: Int,  completion: ((DisplayData?) -> Void)? = nil) {
        ...
        serialAccessQueue.addOperation {
            ...
            self.fetchData(for: identifier, imageId: imageId)
        }
    }
     ...
     */
    private func fetchData(for identifier: UUID, imageId: Int) {
        ...
        if let data = fetchedData(for: identifier) {
            ...
            let operation = AsyncFetcherOperation(identifier: identifier, imageId: imageId)
